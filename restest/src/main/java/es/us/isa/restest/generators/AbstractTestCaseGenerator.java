...
public abstract class AbstractTestCaseGenerator {
	...
	public AbstractTestCaseGenerator(OpenAPISpecification spec, TestConfigurationObject conf, int nTests) {
		...
		// The seed is still recorded (and overridable via setSeed) as upstream does, but
		// it is drawn from RESTestRandom so that RANDOM_SEED makes the run reproducible.
		this.rand = new Random();
		this.seed = RESTestRandom.nextSeed();
